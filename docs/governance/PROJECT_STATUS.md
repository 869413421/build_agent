...
- 阶段：AgentApp 主入口与应用装配层完成（代码 + 测试 + 示例）
- 日期：2026-03-09
...
- 无（`AgentApp` 主入口与应用装配层已完成，下一步进入基于 `AgentApp` 的 CLI / HTTP API 交付）
...
27. [2026-03-09] 已接通 `AgentApp/AgentRuntime` 的工具与 memory 主链路：`allowed_tools` 现在会真实进入模型工具规划与 `ToolRuntime` 执行，`memory` 现在支持前置读取注入与运行后写回；对应定向回归通过，最新全量 `pytest -q` 为 `171 passed`。
28. [2026-03-09] 已收口 `AgentRuntime/ToolRuntime` 的并发边界：`AgentResult.metadata.tool_records` 改为当前 run 的 `state.tool_results` 计数，不再依赖共享 runtime 历史快照；`ToolRuntime` 的幂等缓存与执行记录已增加状态锁，并新增并发回归测试；最新全量 `pytest -q` 为 `183 passed`。

## 下一步唯一任务

- 若本步审核通过，进入基于 `AgentApp` 的 CLI 与 HTTP API 交付，确保应用层统一建立在 `AgentApp -> Agent -> AgentRuntime` 之上。
...
- 2026-03-08 第四章已开始质量回收：`tutorial-quality-checker` 明确抓到 3 处真实代码漂移，分别位于 `support/config/settings.py`、`model_runtime/infrastructure/adapters/base.py`、`tests/unit/test_model_runtime.py` 对应代码块。
- 2026-03-08 第四章当前已完成第一轮漂移修复：上述 3 处代码块已重新对齐主线源码，同时补回 `deepseek_adapter.py` 文件块前缺失的创建命令；章节结构检查通过，新的代码块基线 `code_block_guard verify` 通过（before=80, now=80）。
- 2026-03-08 本轮为避免 Markdown 代码块被测试字符串中的 ```json 截断，已把 `tests/unit/test_model_runtime.py` 中的破损 JSON 构造改为等价的 `fence = \"`\" * 3` 写法；`py_compile` 已通过，`pytest tests/unit/test_model_runtime.py -q` 仍因本机缺少 `openai` 依赖在收集阶段失败，非本轮引入。
- 2026-03-08 第四章已完成第二轮讲解加厚：补入扩展术语地图（`payload / response_schema / json_schema / json_object / extra_kwargs`）、`base.py` 的“第一次阅读先抓 5 个函数点”指引，以及 `test_model_runtime.py` 的“先看这 5 条测试”入口与运行时边界解释。
- 2026-03-08 第四章第二轮加厚后再次通过 `tutorial-quality-checker` 与 `code_block_guard verify --before .tmp/ch04-code-inventory-v3.json`（before=80, now=80），确认本轮只追加讲解、未删改既有代码块。

- 2026-03-08: ??????????????????? `Executor/ChainRunner/??` ?????`tutorial-quality-checker` PASS?`code_block_guard verify` PASS?before=40, now=40??

- 2026-03-08: ??????????????? `??/????/????/????/?? DoD` ???`tutorial-quality-checker` PASS?`code_block_guard verify` PASS?before=36, now=36??

- 2026-03-08: ??????????????? `Agent ????/Agent vs ????? vs ???/???????????` ??????`tutorial-quality-checker` PASS?`code_block_guard verify` PASS?before=36, now=36??

- 2026-03-08: ??????????????? `??????? + ????`??? trace/metrics/replay ?????`tutorial-quality-checker` PASS?`code_block_guard verify` PASS?before=31, now=31??

- 2026-03-08: ?? 1-6 ???????????????????????? `Model Runtime ???? + ????`?`tutorial-quality-checker` PASS?`code_block_guard verify` PASS?before=80, now=80??

- 2026-03-08: ???? 1-6 ????????????? `Protocol ????`???? `Tool Runtime ????`???? `Observability ????`????? `tutorial-quality-checker` ? `code_block_guard verify` ????

- 2026-03-08: ??? 1-6 ??????????????? `14 ?????/????` ???? `????`????? `tutorial-quality-checker` ? `code_block_guard verify` ????

- 2026-03-08: ??? 1-8 ?????? 1-6 ??????????? 7/8 ?????????? `tutorial-quality-checker` ????????????????? 7/8 ? `code_block_guard verify` ????????????

- 2026-03-08: ?? `tutorial-quality-checker` ??????????????????? `???????? codex/bash/powershell` ?????? 1-8 ????????? PASS?

- 2026-03-08: ?????????????????? `????????...` ???? `????`??????????? `tutorial-quality-checker` ? `code_block_guard verify` ????

- 2026-03-08: ?????? `.agents/skills/publish-grade-article-writer/`?????????????????????????? `SKILL.md`?`references/repo-article-standard.md`?`agents/openai.yaml`???????????

- 2026-03-08: ?? `publish-grade-article-writer` ????? `references/article-checklist.md` ? `scripts/run_article_checks.py`?????????????????

- [2026-03-08] Memory 组件代码已完成首版落地：新增 MemoryRuntime + MemoryExtractor + InMemorySessionMemoryStore + InMemoryLongTermMemoryStore + ChromaMemoryVectorStore，支持 
inish/fact/preference 三类 LLM 结构化抽取、结构化双层记忆写入、向量库存储与上下文桥接。
- [2026-03-08] Memory 回归已通过：uv run --no-sync pytest tests/unit/test_memory.py tests/unit/test_memory_chroma.py tests/unit/test_memory_context_bridge.py tests/unit/test_memory_demo.py -q 通过（17 passed）；全量 uv run --no-sync pytest -q 通过（109 passed）。
- [2026-03-08] ????? Memory ????????? `scope=None` ????????????? session memory ????? `session_id` ?? scope ???????????
- [2026-03-08] ?? `ChromaMemoryVectorStore.invalidate()` ? metadata ???????????? metadata ?????????????? `scope/category/memory_id/record_key` ????????
- [2026-03-08] ????????????`ExtractedMemoryItem` ????? `source_type/source_id`??? fact ????????? `tool_result`?`examples/memory/memory_vector_demo.py` ? fake collection ???????? where ???
- [2026-03-08] Memory ???????`uv run --no-sync pytest tests/unit/test_memory.py tests/unit/test_memory_chroma.py tests/unit/test_memory_context_bridge.py tests/unit/test_memory_demo.py tests/unit/test_memory_vector_demo.py -q` => 22 passed??? `uv run --no-sync pytest -q` => 114 passed?

- [2026-03-08] ??? Memory ???????`scope=None` ?????????? `session_id` ????? session + long_term?? `session_id` ??? long_term?????????????????????? session memory ???
- [2026-03-08] ?????? `ChromaMemoryVectorStore.invalidate()` ????????? collection ?? `get+update` ?????????? `delete` ???????????? update?? get??????????? metadata ???
- [2026-03-08] ??? Memory ??????? `????` ???????????????????????Memory ???? 25 passed????? 117 passed?

- [2026-03-08] ??????????? `docs/tutorials/09-?0?1???Agent????-???-Memory-???????????.md`?????? Memory ???????????Chroma ?????bridge?examples ???????????????
- [2026-03-08] ???????????`tutorial-quality-checker` ???`code_block_guard` ??? `.tmp/ch09-code-inventory-v2.json` ?????????Memory ???? `25 passed`?
- [2026-03-08] 第九章 Memory 教程已完成一轮质量回收：修复源码、example、测试代码块中的问号文本，新增 `examples/memory/memory_bridge_demo.py` 与 `tests/unit/test_memory_bridge_demo.py`，并把“记忆读取 -> bridge -> 上下文消息”正式纳入主线。
- [2026-03-08] 第九章最终验证已通过：`tutorial-quality-checker` PASS；`code_block_guard` 基线 `.tmp/ch09-code-inventory-v4.json` verify PASS；Memory 定向回归 `26 passed`；全量回归 `118 passed`（保留 `.pytest_cache` 非阻塞 warning）。
- [2026-03-08] ??? Memory ?????????????? extractor ????????runtime ????????bridge ????????Chroma ?????????? example ????????????
- [2026-03-08] ??? Memory ????????????????????Chroma ??????bridge ???example ?????/??/????????????????????????
- [2026-03-08] ??? Memory ?????????????? 4 ????????????????????????????????????????????????

- [2026-03-08] 已完成第十章 Evaluator 首版代码实现：新增 EvaluatorRuntime、RuleBasedEvaluator、ModelRuntimeJudgeEvaluator，支持 output / trajectory / combined 三种评估模式，并补齐 examples/evaluator 与对应单测。
- [2026-03-08] Evaluator 当前统一走结构化评估结果：规则评估与 LLM judge 都返回 EvaluationResult，compare(...) 可直接比较多个结果；接口文档 docs/architecture/interfaces.md 已同步升级。
- [2026-03-08] 本轮已修复一个真实评估漏洞：空答案此前会因缺少 expected_answer / reference_facts / rubric 而被错误判为高分；现已改为‘空答案必低分、缺参考信息只给中性分’。
- [2026-03-08] Evaluator 定向回归已通过：UV_CACHE_DIR=D:\code\build_agent\.tmp\uv-cache uv run --no-sync pytest tests/unit/test_evaluator.py tests/unit/test_evaluator_judge.py tests/unit/test_evaluator_demo.py tests/unit/test_evaluator_compare_demo.py -q => 9 passed；全量回归 uv run --no-sync pytest -q => 127 passed。

- [2026-03-08] Evaluator 二轮质检修复已完成：RuleBasedEvaluator 与聚合逻辑现正式支持 rubric.dimensions / rubric.weights / pass_threshold；不再忽略公开 rubric 契约。
- [2026-03-08] Evaluator compare 语义已收口：compare(...) 现在返回 winner 的 trace_id / run_id / evaluator / mode / score / verdict，不再只返回 evaluator 名称，适用于同一 evaluator 下的 run-to-run 比较。
- [2026-03-08] LLM judge prompt 已从 Python repr 改为稳定 JSON 注入，并把 rubric 维度与权重写入 system prompt；本轮定向回归 10 passed，全量回归 128 passed。

- [2026-03-08] Evaluator 三轮质检修复已完成：trajectory 里的 replan 计数改为按唯一 revision 去重，不再因同一轮 replan 出现在多个事件里而重复扣分。
- [2026-03-08] compare(...) 现补齐稳定 candidate_id：当 trace_id/run_id 缺失时，会退化为 evaluator:mode:index，避免 winner 无法定位；规则评估侧的 final_answer.output 也已改为 JSON 序列化文本，与 judge 输入格式保持一致。
- [2026-03-08] 本轮定向回归 12 passed，全量回归 130 passed。
- [2026-03-08] 第十章 Evaluator 教程已完成：新增 `docs/tutorials/10-从0到1工业级Agent框架打造-第十章-Evaluator-结果评估与轨迹评分.md`，覆盖 EvaluatorRuntime、RuleBasedEvaluator、ModelRuntimeJudgeEvaluator、examples 与测试主线。
- [2026-03-08] 第十章 Evaluator 教程已完成验证：`tutorial-quality-checker` PASS；`code_block_guard` 基线 `.tmp/ch10-code-inventory-v1.json` verify PASS；教程正文已修复 UTF-8 中文编码与绝对路径链接问题。
- [2026-03-08] 第十章 Evaluator 教程已完成一轮精修：补充“第一次阅读扶手”、更多 runtime/rules/judge 业务例子，以及测试与 example 的使用顺序说明；结构检查与代码块守卫继续通过。
- [2026-03-08] 已完成第十一章 Safety Layer 代码交付：新增 `src/agent_forge/components/safety/` 三层结构、`examples/safety/safety_demo.py` 与 Safety 全套单测；工具阶段通过 `SafetyToolRuntimeHook` 接入 `ToolRuntime before_execute`。
- [2026-03-08] Safety 质检修复已完成：1) `enabled=False` 规则现在会真实跳过；2) Safety hook 会优先消费 `execute(..., capabilities=...)` 的实时能力集合；3) `before_execute` 拒绝已纳入 ToolRuntime 统一错误收口，不再绕过 records 与 observability。
- [2026-03-08] Safety 验证已通过：`uv run --no-sync pytest tests/unit/test_safety.py tests/unit/test_safety_tool_hook.py tests/unit/test_safety_demo.py -q` => 14 passed；`uv run --no-sync pytest -q` => 144 passed；第十一章教程已写入并同步 README 索引。
- [2026-03-09] 已完成 `AgentApp` 应用装配层：新增 `src/agent_forge/runtime/app.py`，公开 `AgentApp / AgentAppTool`，支持注册 model、tools、memory、retrieval、evaluator、safety，并通过 `create_agent(...)` 返回 `Agent`。
- [2026-03-09] `AgentApp` 当前采用“全局注册 + agent 选择”工具装配模型：`register_tools([...])` 负责登记工具池，`create_agent(..., allowed_tools=[...])` 只为当前 agent 授权工具子集；默认内置 `default` 本地模型，保证主入口可开箱即用。
- [2026-03-09] 已为内置 `PythonMathTool / TavilySearchTool` 增补 `tool_spec`，使其可被 `AgentApp.register_tools([...])` 直接消费；新增示例 `examples/agent/agent_app_demo.py` 与定向回归 `tests/unit/test_agent_app.py / tests/unit/test_agent_app_demo.py`。
- [2026-03-09] 已完成 `AgentApp` 二轮质检修复：鉴于 `AgentRuntime` 主链路尚未接通 tool planning/tool execution 与 memory 读写闭环，`create_agent(..., allowed_tools=...)` 与 `create_agent(..., memory=...)` 现改为 fail fast，避免对外暴露静默 no-op 能力；新增回归后全量 `pytest -q` 为 168 passed。
- [2026-03-09] 已完成 `AgentRuntime` 五轮质检修复：`tool_call_id` 现自动追加 `run_id` 命名空间，避免同一 agent 多次运行时误命中旧的 ToolRuntime 幂等缓存；`memory_write_count` 现优先使用 `MemoryWriteResult.structured_written_count/vector_written_count` 统计，不再仅依赖 `len(records)`；新增回归后最新全量 `pytest -q` 为 178 passed。
- [2026-03-09] 已完成 `AgentRuntime/Memory` 六轮质检修复：失败 run 不再写入 `finish` memory，避免把失败兜底答案沉淀进记忆；`fact` memory 现在只消费成功的工具结果，失败工具输出不会再进入事实抽取；新增定向回归后最新全量 `pytest -q` 为 181 passed。
- [2026-03-09] 已完成 `AgentApp/AgentRuntime` 三轮质检修复：`AgentRuntime` 现支持在 `parsed_output` 缺失时从 `ModelResponse.content` 做 JSON/纯文本兜底收口，避免第三方 ModelRuntime 只返回文本时被静默吞掉；`AgentApp.register_memory(...)` 现明确只接受具备 `read/write` 的 memory runtime，并在注册阶段立即校验；新增定向回归后最新全量 `pytest -q` 为 175 passed。
- [2026-03-09] 已完成 `AgentRuntime` 四轮质检修复：step/tool 失败现在会从 Engine 错误事件中提取 `ErrorInfo` 并抬升到最终 `AgentResult.error`；`metadata.tool_records` 改为“本次 run 的增量工具执行数”，不再复用同一 agent 的历史累计记录；新增定向回归后最新全量 `pytest -q` 为 177 passed。

- [2026-03-09] 已完成 runtime 公开层编码收口：重写 src/agent_forge/runtime/ 下的 __init__.py / schemas.py / agent.py / app.py / defaults.py / runtime.py，以及 src/agent_forge/components/tool_runtime/application/runtime.py，清理中文乱码、恢复主流程步骤注释与用户可见错误文案；定向回归 48 passed，全量回归 183 passed。

- [2026-03-09] 已完成一轮教程与生产状态审计：第 12 章 	utorial-quality-checker 与 un_article_checks.py 均确认代码漂移，README 仍存在编码污染；已新增 docs/governance/TUTORIAL_ALIGNMENT_AUDIT.md 与 docs/governance/PRODUCTION_READINESS_CHECKLIST.md，当前对外结论固定为‘教程暂不公开、框架暂不投入生产’。

- [2026-03-09] 已完成第一轮公开文档同步：重写 README.md 的仓库首页说明与 Quickstart，明确当前主入口为 AgentApp、Agent 为轻量直用路径；同时修复 examples/agent/agent_demo.py 与 examples/agent/agent_app_demo.py 的中文乱码，相关 demo 测试 2 passed，全量回归保持 183 passed。

- [2026-03-09] 教程同步收口：已重写第12章 AgentApp/Agent/AgentRuntime 教程，并修平第05章 ToolRuntime、第09章 Memory 的代码漂移；tutorial-quality-checker 已通过 05/09/11/12。
- [2026-03-09] 已完成第 3 章 Engine 教程的一轮“降认知负担”精修：在不改动组件代码的前提下，重写开头阅读入口，新增“带着问题读”主线、阅读导航，以及 `loop.py` 源码前的阅读抓手，重点消除术语堆叠和补丁式过渡带来的阅读割裂感；`tutorial-quality-checker` 已再次通过。
