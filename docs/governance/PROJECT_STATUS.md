...
- [2026-03-08] Memory 组件代码已完成首版落地：新增 MemoryRuntime + MemoryExtractor + InMemorySessionMemoryStore + InMemoryLongTermMemoryStore + ChromaMemoryVectorStore，支持 inish/fact/preference 三类 LLM 结构化抽取、结构化双层记忆写入、向量库存储与上下文桥接。
