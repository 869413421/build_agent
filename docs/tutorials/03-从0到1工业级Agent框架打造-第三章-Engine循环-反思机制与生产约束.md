...
> 如果你第一次读第三章就觉得“概念很多、每个都懂一点、但串不成一条线”，先别急着从头硬读到尾。第三章真正要解决的不是“再定义几个对象”，而是“前面已经有协议对象了，为什么系统还是跑不起来”。
...
但第三章最容易让人读着发紧的地方，其实不是代码，而是术语和机制一下子变多了。前两章我们刚把 Protocol 讲清楚，这一章又同时引入 `ExecutionPlan`、`reflect`、`replan`、attempt pipeline、commit point、PlanAudit。很多读者不是看不懂单个词，而是刚把前一个词放进脑子里，后一个词又来了，于是整章就会带一点“像在读多次补上去的说明”的观感。

所以这次先换一个读法，不急着扎进源码。你先把第三章当成在回答一个非常具体的问题：

> 前面我们已经有了 `AgentState` 这些协议对象，为什么还不够？为什么系统还必须再补一个 Engine？

答案其实很朴素：因为“有状态对象”和“能稳定跑完一轮任务”不是一回事。只有把 `plan -> act -> observe -> reflect -> update -> finish` 这条链落成真实执行内核，后面的 Model Runtime、Tool Runtime、Observability、Context Engineering 才有地方挂载。否则整个框架会一直停留在“对象设计得还行，但没有真正执行骨架”的阶段。
...
1. 把 Engine 从“一个大循环”拆成“可插拔的执行管线”。
2. 把一次任务从“按顺序跑步骤”升级成“带反思、可重试、可重规划的正式执行过程”。

建议你一直带着这 4 个问题读第三章：

1. 这段内容是在回答“做什么”，还是在回答“为什么这样做”？
2. 这段机制是在保护“当前步骤”，还是在保护“整轮计划”？
3. 这里说的失败，应该 `retry`，还是应该 `replan`？
4. 这段代码是在补业务能力，还是在补执行骨架？

后面所有对象和辅助函数，基本都可以往这两条主线上挂。

如果你一上来就觉得这一章信息量很大，先只记住这 3 句话：

1. `ExecutionPlan` 解决的是“任务不能只有步骤，还得有目标、约束、成功标准和风险信息”。
2. `pipeline engine` 解决的是“Engine 不能继续写成一个越来越大的 while 循环”。
3. `replan` 解决的是“不是每次失败都该重试，有些时候应该改计划本身”。

只要先记住这三句，后面看到 `PlanAudit`、`attempt pipeline`、`commit point` 这些词时，就不会觉得它们是凭空冒出来的。
...
如果你已经翻过一次第三章，又觉得信息很多但抓不住重点，建议这次按下面顺序读，而不是从头到尾硬顶：

1. 先看“整个 Engine 主链路”和“单步 attempt 链路”两张图，先建立主流程全景。
2. 再看下面的“名词速览”，只记住 `ExecutionPlan / PlanStep / reflect / replan / commit point` 这几个核心词。
3. 接着直接跳到 `第 6 步` 的 `loop.py`，先只关心一件事：一轮执行到底是怎么推进和停下来的。
4. 最后再回来看 `helpers.py` 和 `test_engine.py`，把“为什么这么设计”和“测试在保护什么”补齐。

换句话说，这一章不是“每个小节权重都一样”的文档。真正的阅读重心是：

1. 先看主流程。
2. 再看执行决策。
3. 最后看辅助规则和测试护栏。

这样读，会比一开始就把每个结构体字段都读透轻松很多。
...
如果你觉得 `ExecutionPlan`、`replan` 这些词还是抽象，可以先把 Engine 想成“带复盘机制的项目执行器”。

### 生活化例子

假设老板给你一个全局任务：

> 明天下午 3 点前，给客户交一版可讨论的投标方案。
...
1. `global_task`：明天下午 3 点前交一版可讨论的投标方案
2. `ExecutionPlan.steps`：收集需求、整理材料、输出方案
3. `constraints`：不能超时、不能遗漏关键报价项
4. `success_criteria`：客户能看懂、能继续开评审会
5. `risk_level`：如果客户需求很模糊，这轮计划风险就高
...
所以这一章不是在讨论“怎么写一个复杂类”，而是在讨论：

**当任务发生变化时，Engine 有没有能力正式改计划，而不是只会死跑原步骤。**

如果你把这句话记住，后面那些看起来很多的对象，其实都只是围绕这个目标服务：

1. `ExecutionPlan` 负责把“计划”定义清楚。
2. `helpers.py` 负责把“计划规则”收口清楚。
3. `loop.py` 负责把“执行推进和分支决策”跑清楚。
4. `test_engine.py` 负责把这些行为锁成回归护栏。
...
走到这里，阅读方式最好再切一次。前面的 `schemas.py`、`context.py`、`helpers.py` 其实是在给 `loop.py` 清障：把计划对象、运行态共享数据、计划规则收口都提前拆出去。这样你现在读 `loop.py` 时，不需要再同时理解所有细节，只要先盯住下面 4 个问题：

1. top-level pipeline 是怎么从 `plan` 走到 `finish` 的？
2. 单步 attempt pipeline 是怎么从 `act` 走到 `decide` 的？
3. 什么情况下会停在当前 step，什么情况下会推进到下一个 step？
4. `retry` 和 `replan` 到底分别改了什么？

你会发现，`loop.py` 真正承担的职责不是“什么都做”，而是“把已经定义好的规则按顺序驱动起来”。前面拆出去的越干净，这里就越容易读。
...
### 第一次读 `loop.py`，先只抓这 4 件事

如果你第一次读 `loop.py` 就直接顺着每个私有方法往下啃，很容易迷路。先只抓这 4 件事：

1. `arun()` 是总入口，它负责把一次 run 的公共上下文搭起来。
2. 顶层 pipeline 只关心 3 段：`plan -> execute_steps -> finish`。
3. `execute_steps` 里面才会进入单步 attempt pipeline：`act -> observe -> reflect -> decide`。
4. 只有走到 `update`，这一步才算真正提交成功。

先把这 4 个判断抓住，再看细节，会轻很多。

### 第二次读 `loop.py`，再看这 4 个细节

第二遍再补这 4 个点：

1. 为什么恢复跳过靠 `stable step key`，不是靠步骤索引。
2. 为什么 `replan` 要提升 `revision`，而不是直接把剩余列表改掉。
3. 为什么计划调度要在进入执行前就完成，而不是边跑边碰运气。
4. 为什么 `finish` 不只是收尾，而是把整轮 run 的统计和治理信息统一收口。
...
`test_engine.py` 很长，但第一次不用全看。先抓这 5 条，基本就能理解 Engine 到底在保护什么：

1. `test_engine_run_success_flow()`：证明主链路能闭环跑通。
2. `test_engine_resume_uses_stable_step_key_not_idx()`：证明恢复不是靠脆弱索引。
3. `test_engine_should_schedule_steps_by_dependencies_and_priority()`：证明计划字段真的进入执行语义。
4. `test_engine_should_replan_remaining_steps()`：证明失败后可以正式改计划，而不是只会重试。
5. `test_engine_should_inherit_risk_and_audit_creator_when_replan_omits_them()`：证明计划治理字段不会被默认值偷偷污染。

如果这 5 条都看懂了，这一章的主线其实已经抓住了。
