...
## 如果你第一次接触 Context Engineering，先记这 3 句话

1. Context Engineering 不是“把消息拼起来”，而是“决定哪些内容能进模型、哪些内容必须被裁掉、为什么裁掉”。
2. 它解决的是预算、优先级和可解释性问题，不是模型推理问题。
3. 如果没有这一层，长会话、工具 schema、检索引用一叠加，模型输入很快就会失控。

第一次读这一章时，先抓住这条主线：

`原始请求 -> ContextEngineeringRuntime -> ContextBundle -> ModelRuntime`

先看懂“为什么不能把所有上下文无脑塞给模型”，再去看预算估算、裁剪策略和 hook 接线，会轻松很多。

## 名词速览

第一次看这一章，最容易混的不是代码，而是这些词各自到底处在哪一层：

1. `Context Budget`：上下文预算。它定义这次请求最多还能放多少内容。
2. `Context Bundle`：编排后的最终产物，是真正准备送进模型的上下文集合。
3. `Budget Report`：预算报告。它记录哪些内容保留了、哪些被裁了、为什么裁。
4. `Citation`：来自检索或外部知识的引用材料，本章关注的是“它最终有没有真正进入模型”。
5. `Trimming Policy`：裁剪策略。它决定超预算时先删谁、后删谁。
6. `Token Estimator`：估算器。它不负责真的调用模型，只负责对上下文大小做稳定、可测的估算。
7. `Hook`：接入点。本章把 Context Engineering 放在 `ModelRuntime` 请求前，而不是塞进 Engine。

这一章最关键的一句话是：

> 上下文工程的目标不是“尽量多塞内容”，而是“在有限预算内，把最值得进模型的内容稳稳送进去”。
