...
很多读者第一次看这一章时，不是卡在 Python 语法，而是卡在“这些 Runtime 术语到底各自负责什么”。先把这一章最容易混的词压住：

1. `ToolCall`：上游发下来的结构化工具调用请求。
2. `ToolResult`：工具执行后的标准化返回，不管成功失败都要回到这个结构里。
3. `ToolSpec`：工具规格说明书，定义参数 Schema、权限、敏感字段、超时等执行约束。
4. `Executor`：单次工具执行引擎，负责校验、超时、重试、记录和错误收口。
5. `ChainRunner`：链式编排器，负责把前一步输出安全接到下一步输入。
6. `Facade`：门面层，对外收口统一接口，对内隐藏执行细节。
7. `Hook`：生命周期扩展点，用来插入观测、审计、策略，而不是改主执行逻辑。
8. `Idempotency`：幂等，同一个调用重复到来时不应该重复造成副作用。
9. `Isolation`：隔离执行，这里指“工具失败必须被收口，不能直接击穿 Agent 主流程”。

这一章记住一句最关键的话：

> Tool Runtime 不是“帮你调函数”的工具箱，而是“替 Agent 扛住函数调用风险”的执行边界层。

## 如果你第一次接触 Tool Runtime，先记这 3 句话

1. Tool Runtime 不是为了“把函数调起来”，而是为了“把函数安全、稳定、可追踪地调起来”。
2. `Executor` 解决的是一次工具调用怎么安全执行，`ChainRunner` 解决的是多次工具调用怎么串起来。
3. 如果没有这一层，Engine 一旦直接碰真实工具，超时、脏参数、幂等、副作用和失败收口都会迅速失控。

第一次读这一章时，先抓住这条主线：

`ToolCall -> Executor -> ToolResult`

先看懂单次调用为什么要有校验、超时、错误收口，再去看链式编排、hook 和异步执行，会轻松很多。
...
很多人读到这里时，会把 `ToolRuntime`、`Executor`、`ChainRunner`、`HookDispatcher` 当成一堆名字相近的执行类。其实它们的分工非常清楚：

```mermaid
flowchart TD
  A[上游传入 ToolCall 或 ToolChainStep] --> B[ToolRuntime 门面]
  B --> C[Executor 单次执行]
  B --> D[ChainRunner 链式编排]
  C --> E[HookDispatcher 生命周期分发]
  C --> F[具体工具实现]
  D --> C
```
...
1. `ToolRuntime` 像总服务台，外部只和它打交道。
2. `Executor` 像安检加登机口，负责检查、放行、拦截和异常处理。
3. `ChainRunner` 像中转调度，负责把上一段输出安全交给下一段。
4. `HookDispatcher` 像黑匣子和监控探针，不决定流程去哪，但会记录、插桩、拦截异常事件。

这个分工非常关键，因为它解释了为什么这一章不能把所有逻辑都塞进一个 `runtime.py`。
...
文件：[src/agent_forge/components/tool_runtime/domain/schemas.py](../../src/agent_forge/components/tool_runtime/domain/schemas.py)
...
文件：[src/agent_forge/components/tool_runtime/domain/__init__.py](../../src/agent_forge/components/tool_runtime/domain/__init__.py)
...
文件：[src/agent_forge/components/tool_runtime/application/utils.py](../../src/agent_forge/components/tool_runtime/application/utils.py)
...
文件：[src/agent_forge/components/tool_runtime/application/hooks_dispatcher.py](../../src/agent_forge/components/tool_runtime/application/hooks_dispatcher.py)
...
所以 `HookDispatcher` 的价值不是“再包一层”，而是把“主执行逻辑”和“外挂能力”切开。  
后面你接 Observability、审计、策略风控时，才能不把 `Executor` 写成什么都管的大类。
...
1. `step`：链路里的一个执行节点。
2. `binding`：把前一步结果映射到当前步骤参数的规则。
3. `context`：整条链在运行过程中累积出来的中间态。

你可以把 `binding` 理解成“接线图”，把 `context` 理解成“中转面板”。  
`ChainRunner` 做的事，本质上就是按照接线图，从中转面板里把值取出来，再送进下一个工具。
4. 长链路要防止上下文膨胀：  
...
文件：[examples/tool_runtime/tool_runtime_demo.py](../../examples/tool_runtime/tool_runtime_demo.py)
...
### 第 8 步：补齐共享测试夹具 `conftest.py`

在创建 `conftest.py` 之前，先把测试目录显式声明为 Python 包，避免 `from tests.unit...` 导入时报错。
...
文件：[tests/__init__.py](../../tests/__init__.py)



```python
"""Tests package marker for absolute imports."""
```
...
文件：[tests/unit/__init__.py](../../tests/unit/__init__.py)



```python
"""Unit tests package marker for absolute imports."""
```
...
文件：[tests/unit/conftest.py](../../tests/unit/conftest.py)
...
1. `test_tool_runtime.py` 里会 `from tests.unit.conftest import FakeTavilyClient`，所以必须先有这个文件。
2. 这个假客户端保证测试在离线环境也能跑，不依赖真实 Tavily 网络请求。
3. 先创建 `conftest.py` 再写测试文件，可以避免“按教程执行却导入报错”。

---

### 第 9 步：补齐单元测试 `test_tool_runtime.py`

```codex
mkdir -p tests/unit
touch tests/unit/test_tool_runtime.py
```

文件：[tests/unit/test_tool_runtime.py](../../tests/unit/test_tool_runtime.py)
...
1. 这组测试覆盖了 Tool Runtime 的核心风险面：参数校验、权限控制、幂等缓存、超时、hook、链式编排、异步并发。
2. 真正避免“教程能写不能跑”的关键，是先明确创建 `tests/unit/test_tool_runtime.py`，再执行 `uv run pytest ...`。

### 测试为什么重要

这一章的测试，不是在证明“某个函数今天返回对了”，而是在锁 Tool Runtime 这一层最关键的行为护栏：

1. 错误要被结构化收口，不能直接把主流程炸穿。
2. 重复调用要能被幂等控制住，不能重复造成副作用。
3. 链式绑定要能稳定传值，不能因为一步结构变化整条链失真。
4. Hook 要真的接在生命周期上，而不是定义了接口但从来没被调到。

所以这组测试的读法不能停留在断言层面，而要把它当成“执行边界的回归合同”。
...
1. 单点调用闭环：`ToolRuntime.execute()` 能把 `ToolCall` 稳定转换成 `ToolResult`，而不是把底层异常直接炸到主流程。
2. 链式执行闭环：`ChainRunner` 能把前一步输出稳定传给下一步，不会因为 JSON Path 绑定错误把整条链打断。
3. 可观测闭环：Hook 能在 `before_execute / on_event / on_error / after_execute` 四个阶段都拿到可调试的信息。

## 验证清单

当你完成本章后，至少要逐项确认下面这些事情：

1. 你能说清 Tool Runtime 在系统里的位置：它是动作执行层，不是 Engine，也不是 Model Runtime。
2. 你能说清 `ToolCall -> Executor -> ToolResult` 这条主链路是如何跑通的。
3. 你能说清为什么要同时保留 `Executor`、`ChainRunner`、`Facade`，而不是把所有逻辑塞进一个大类。
4. 你能独立运行 `examples/tool_runtime/tool_runtime_demo.py`，并理解单次执行与链式执行的差别。
5. 你能独立运行 `tests/unit/test_tool_runtime.py`，并知道这些测试在锁哪些行为护栏。
6. 你能解释为什么 Hook、超时控制、参数校验都要放在 Runtime 这一层，而不是让 Engine 或具体工具自己兜底。
