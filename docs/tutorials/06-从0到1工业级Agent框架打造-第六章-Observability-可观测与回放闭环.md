...
## 如果你第一次接触可观测，先记这 3 句话

1. 可观测不是“多打点日志”，而是让你能回答“刚才到底发生了什么、为什么失败、能不能重放”。
2. `trace` 解决的是“过程怎么看”，`metrics` 解决的是“整体健康度怎么看”，`replay` 解决的是“出事后怎么复盘”。
3. Agent 越复杂，越不能只靠 `print` 排障；没有 Observability，后面的 Tool、Context、Retrieval 都会越来越难维护。

如果你现在只先记住这 3 句话，已经足够进入第六章主线。后面再看 `runtime / hook / sink / replay` 这些术语时，就不容易迷失。

## 名词速览：先把可观测讲成人话

第一次看这一章，最容易被 `trace / metrics / replay / sink / hook` 这些词压住。先用最直白的话把它们压平：

1. `trace`：一条任务从开始到结束的时间线，你可以把它理解成“这次运行的行动轨迹”。
2. `metrics`：聚合后的统计数字，比如成功率、错误数、平均耗时、P95。
3. `replay`：把当时每一步做了什么重新拼出来，重点不是“统计”，而是“复盘”。
4. `sink`：数据落点。比如把 trace 写进内存、文件或外部系统。
5. `hook`：接线点。业务在不改主逻辑的前提下，把事件送进可观测系统。
6. `sampling`：采样。不是所有成功请求都全量保留，而是按策略保留一部分，防止观测成本失控。
7. `redaction`：脱敏。把 token、秘钥、用户敏感字段在落盘或导出前处理掉。

这一章的核心不是“会不会记日志”，而是：

> 怎么把 Agent 的运行过程，收口成一套能看、能查、能回放、能导出的观测闭环。

## 如果你第一次读第六章，先抓这 4 段

第一次读这一章，不建议一上来就扎进 `runtime.py` 或所有 store 细节。先按这个顺序抓主线：

1. 先看“主流程时序图”，搞清楚 Engine 事件和 Tool 事件是怎么汇到同一条观测链路里的。
2. 再看 `ObservabilityRuntime`，理解它为什么是这一章真正的收口点。
3. 再看 `hooks.py`，理解 Tool Runtime 为什么不是直接写 trace，而是通过 hook 接进来。
4. 最后再看 `memory.py` 和测试，理解“数据怎么存”和“为什么这些行为能被证明正确”。

也就是说，第一次读这一章的顺序应该是：

`主流程 -> Runtime -> Hook -> Store -> Tests`

先抓“链路怎么连”，再抓“每个类怎么写”，阅读负担会明显下降。
